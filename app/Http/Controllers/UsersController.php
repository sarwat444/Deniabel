...
class UsersController extends Controller
{
    ...
    public  function forgetpassword(Request $request)
    {
        if($request->isMethod('post'))
        {
            ...
            if ($usercount == 0 ){
                return  redirect()->back()->with('flash_message_error' ,  'This Email Is Not  Found') ;  
            }
           else 
           {
             $userdetails =  User::where('email' , $data['email'])->first() ; 
            $encrybtpass  = str_random(8) ;
            $newpassword = app('hash')->make($encrybtpass); 
             ...
             $messageData = [
                 'email'  => $data['email'] , 
                 'name'   => $name , 
                 'password'  => $encrybtpass
             ]; 
             Mail::send('emails.forgetpassword' ,  $messageData , function($message)  use($email){

             $message->to($email)->subject('Your New Password - Deniabel Website') ;

             });
             return redirect('login-register')->with('flash_message_success' , 'Please Check  your email For Your Password') ;  
